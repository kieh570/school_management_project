...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $required = ['first_name', 'last_name', 'dob', 'pob', 'address', 'nationality', 'qualification', 
                 'contact', 'role', 'email', 'password', 'confirm_password', 'gender'];
    ...
    if (!empty($errors)) {
        ...
        function generateUniqueTeacherID($conn) {
            ...
            do {
                ...
                $stmt = $conn->prepare("SELECT id FROM admin_table WHERE id = ?");
                $stmt->execute([$id]);
                $attempts++;
                if ($attempts > 50) {
        ...
        $stmt = $conn->prepare("INSERT INTO admin_table 
            (id, first_name, middle_name, last_name, dob, pob, address, nationality, qualification, 
             contact, role, email, signup_date, password, gender) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        $success = $stmt->execute([
            $id, $first_name, $middle_name, $last_name, $dob, $pob, $address, 
            $nationality, $qualification, $contact, $role, $email, $signup_date, $hashed_password, $gender
        ]);

 if ($success){  
    echo"<h1>Admin Signup successful. $first_name ID is: $id</h1>";
    } else {
